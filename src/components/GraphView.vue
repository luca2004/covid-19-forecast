    ...
    <div class="graphview">
      <div class="row">
        ...
        <div class="row">
            ...
            <el-date-picker placeholder="Enter End Date" class="input" v-model="dateend" type="date" >
            </el-date-picker>
        </div>
        ...
        <div class="row">
            ...
            <el-input placeholder="" class="input" :value="date_to_peak" readonly="readonly"></el-input>  <!-- v-model="mean" -->
        </div>
        <div class="row" v-show="false">
...
export default {
  ...
  computed:{
    date_to_peak: ( state ) => { return state.add_days_to_startdate( state.mean ); }
  },
  methods:{
    convert_date_to_string(date){
      const __ = (n) => { return n<10?'0'+n:n;}
    ...
    select_country(){
      //console.log(this)
      let graph = new plotter( { selectorId: 'my_graph', startdate: new Date(this.datestart) });    
      graph.clear();
      ...
      if(item){
        ...
        this.dateend = this.add_days_to_startdate( item.mean * 2 );
      }
    },
    ...
    async plot_graph(){
      ...
      let fc = new FunctionCalculator({
        xmin: 0, xmax: this.get_diff_date(), func: this.get_gaussian_function()
      });

      let gaussiana = fc.evaluateFunction();
      let api = new covid19api();
      let data = await api.CountryTimeSeriesData(this.selectedCountryCode, 
     ...
     // console.log(data)
      ...
      graph.plot( gaussiana, true, 'forecast data' );      
      //console.log(data)      
      graph.plot( CCD.get_new_cases_with_media_to_3_gg(), false, 'real data' );
